...
Wachttijd-indicatie: Een implementatie MAG bij een blokkade met reden `EMPTY_TABLE_CELL` informatief het eerstvolgende tijdstip tonen waarop de reguliere herhalingsduiksystematiek weer van toepassing is. Dit tijdstip volgt uitsluitend uit het tabeldomein van tabel 4a: het is het moment waarop het oppervlakte-interval de ondergrens bereikt van de eerstvolgende kolom waarin de rij van de geldende herhalingsgroep (na de HG-aanpassing van sectie 5) een waarde heeft. Toont een implementatie deze indicatie, dan MOET het getoonde tijdstip exact met deze tabelgrens overeenkomen. De indicatie is informatief en wijzigt de berekeningsstatus niet: de geblokkeerde duik blijft `BLOCKED` totdat het oppervlakte-interval werkelijk is verstreken en de vervolgduik langs de reguliere weg is berekend. Bronvoorbeelden: geldende herhalingsgroep D bij een oppervlakte-interval korter dan 15 minuten: het eerstvolgende geldige tijdstip ligt op 15 minuten na bovenkomst (kolom 0:15-0:29, herhalingsfactor 1.8). Geldende herhalingsgroep G: de kolom 0:15-0:29 is leeg (herhalingsfactor groter dan 2.0); het eerstvolgende geldige tijdstip ligt op 30 minuten na bovenkomst (kolom 0:30-0:59, herhalingsfactor 1.9).

Handelingsopties: Een implementatie MAG bij deze blokkade de bestaande vervolgroutes als handelingsopties aan de duikploegleider presenteren: (1) wachten tot het tijdstip van de wachttijd-indicatie; (2) de vervolgduik zo plannen dat het samenstel aan de meterregelvoorwaarden van sectie 4 voldoet, waarna de meterregelweg geldt; (3) niet duiken; (4) handmatige beoordeling, vastgelegd in de vrije notitievelden. De presentatie van handelingsopties is informatief en wijzigt de berekeningsstatus niet. Een berekening van het samenstel als gecombineerde duik buiten de meterregels MOET NIET als geldig rekenresultaat worden gepresenteerd zolang de bron de methode niet dekt (zie de noot hieronder en open punt 6 in sectie 11).
